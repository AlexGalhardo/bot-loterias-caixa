...
const isTimeToExecute = (days: number[], envHour: string, envMinute: string): boolean => {
    ...
    return (
        days.includes(day) &&
        now.getHours() === Number(envHour) &&
        now.getMinutes() === Number(envMinute)
    );
};
...
const scheduleVerifyNumbersLotomania = () => {
    ...
    if (isTimeToExecute(daysToVerifyLotomania, process.env.VERIFY_LOTOMANIA_HOUR, process.env.VERIFY_LOTOMANIA_MINUTE)) {
