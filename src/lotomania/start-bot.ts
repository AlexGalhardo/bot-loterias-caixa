...
export async function startBot(page: Page) {
    ...
    try {
        await page.goto(process.env.LOTERICAS_CAIXA_LOTOMANIA_URL, { waitUntil: "networkidle0" }); // 'domcontentloaded', networkidle2, networkidle0
    ...
    try {
        ...
        TelegramLog.info(`\n\nEnviado código de login das Lotericas CAIXA no seu email!`);
    } catch (error: any) {
        console.log(error.message);
        throw new Error(error.message);
    }
    ...
    try {
        ...
        console.log("Passo 7 -> clicando no botão login")
    } catch (error: any) {
        console.log(error.message);
        throw new Error(error.message);
    }
    ...
    try {
        ...
        await page.waitForFunction(
            () => (document.querySelector("#codigo") as HTMLInputElement)?.value.length > 0,
            { timeout: 0 }
        );
    ...
    try {
        ...
        await page.evaluate(() => {
            const button = Array.from(document.querySelectorAll(".button-group button[tabindex='1']"))
                .find(btn => btn.textContent?.trim() === "Entrar");
    ...
    while (gamesToMade < 7) {
        try {
            ...
            console.log(`Clicked 'Complete o Jogo' for game ${gamesToMade + 1}`);
        } catch (error: any) {
        ...
        try {
            ...
            console.log(`Clicked 'Colocar no Carrinho' for game ${gamesToMade + 1}`);
        } catch (error: any) {
    ...
    try {
        ...
        console.log(`Passo 20 -> Clicou valor concursos`)
    } catch (error: any) {
    ...
    try {
        ...
        TelegramLog.info(`\n\nPagar Novo Jogo Lotomia!\n\nPIX: ${codigoPixValue}\n\nValor total das apostas: ${valorTotalApostas}\n\nConcursos: ${concursos}`);

        console.log(`Passo 21 -> Clicou valor codigoPixValue e enviou dados para o telegram`)
    } catch (error: any) {
